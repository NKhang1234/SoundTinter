...
class RabbitMQ(MessageBroker):
    ...
    async def send(self, userID: str, fileName: str, status: str):
        ...
        logger.info(f"{self.serverName} sent successfully status of {fileName} - user {userID}")
    ...
    async def _callback(self, message: aio_pika.IncomingMessage):
        async with message.process():
            await self.receiveBuffer.put({
                ...
                "contentType": message.headers.get("contentType")

            })
            logger.info(f"{self.serverName} received {message.headers.get('fileName')} from user {message.headers.get('userID')}")
